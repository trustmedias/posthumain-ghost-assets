...
(function () {
  ...
  var CSS = [
    ...
    ".ph-srcm-card{position:relative;width:100%;max-width:440px;background:#0f0f0f;border:1px solid rgba(0,255,119,.28);padding:38px 32px 32px;box-shadow:0 24px 70px rgba(0,0,0,.6),0 0 0 1px rgba(0,255,119,.06) inset;text-align:center;transform:translateY(14px) scale(.98);transition:transform .22s ease;font-family:inherit}",
    ".ph-srcm-overlay.is-open .ph-srcm-card{transform:translateY(0) scale(1)}",
    ".ph-srcm-close{position:absolute;top:12px;right:14px;width:34px;height:34px;line-height:34px;background:none;border:none;cursor:pointer;color:#888;font-size:24px;transition:color .15s,background .15s}",
    ".ph-srcm-close:hover{color:#fff;background:rgba(255,255,255,.06)}",
    ".ph-srcm-badge{display:inline-flex;align-items:center;gap:7px;margin-bottom:18px;padding:6px 13px;background:rgba(0,255,119,.10);border:1px solid rgba(0,255,119,.35);color:#00ff77;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase}",
    ...
    ".ph-srcm-btn{display:block;width:100%;padding:14px 20px;margin-top:10px;cursor:pointer;font-size:15px;font-weight:700;font-family:inherit;text-decoration:none;border:1px solid transparent;transition:transform .12s ease,background .15s ease,border-color .15s ease}",
  ...
  document.addEventListener("click", function (e) {
    var link = e.target.closest && e.target.closest("a[data-ph-source]");
    if (!link) return;

    var n = link.getAttribute("data-ph-source");
    if (document.getElementById("source-" + n)) return; /* cible présente → natif */
